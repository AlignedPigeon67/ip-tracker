...
const handleFetchLocationError = (err) => {
  // const
  // alert("Invalid IP address or domain input");
  ipInputElement.classList.add("error");
  setTimeout(() => {
    ipInputElement.classList.remove("error");
  }, 3000);
  console.table(err);
};
...
const fetchIpLocation = async (query = "") => {
  ...
  try {
    ...
    if (isIpFail) handleFetchLocationError([err, currentErr]);
  }
};
